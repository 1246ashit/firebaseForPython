...
def video_detection(path_x):
    ...
    while True:
        ...
        if actionD!=None:
            ...
            frame=drawAndAlert(actionD[0],actionD[1],actionD[2],actionD[3],actionD[4],actionD[5],frame)
        ...
        if fireD!=None:
            fireD=list(fireD)
            frame=drawAndAlert(fireD[0],fireD[1],fireD[2],fireD[3],fireD[4],fireD[5],frame)
        
        #臉部偵測區
        a=facedetect3.runRecognition(frame)
        print(a)
        frame=facedetect3.drawInImg(face_names=a[0],img=frame,face_locations=a[1])
        yield frame